...
Dieser Vektor sollte im optimalen Fall im 90 Grad Winkel zu dem Normalvector einer Fläche liegen, damit die Fläche möglichts parallel zu der Achse der Druckerdüse ist. 
...
\begin{figure}[h]
    \centering
    \begin{math} \sum_{ i = 1 }^{ n }{ x_{i} * w_{i} } = o_{j} \end{math}
\end{figure}

Diese Funktion bildet die gewichteten Eingaben dann auf eine Ausgabe $o_{j}$ ab, 
...
Ein typisches Neuron ist in Abbildung \ref{Funktionsweise eines künstlichen Neurons} dargestellt. Durch das Anpassen von Gewichten,$w_{i}$ in der Abbilding \ref{Funktionsweise eines künstlichen Neurons}, kann ein Neuron den Einfluss von Eingabedaten ($x_{i}$) verändern. 
...
oder festgelegtem Optimum vergleichen kann. In dem Falle ist das richtihe Ergebnis der Normalvector zu der jeweiligen Punktwolke. 
...
Die Extraktrion der Merkmale wird in diesem Abschnitt erklährt.
...
Dieser Vektor wird Merkmalsvektor genannt und ist eine abstrakte Darstellung der Punktwolken. Der Merkmalsvektor ist als Zwischenergebniss wichtig, um die Eingabedaten in für ein Regressionsnetz besser verarbeitbare Form zu bringen. Für Menschen ist der Inhalt eines Merkmalsvektors meistens unverständlich.
...
Wenn die Auswahl der Punkte $A$ ist und die gesammte Punktwolke $P$ ist, dann gilt:

\begin{figure}[h]
    \begin{math} A \Subset P\end{math}
    \begin{math} \forall \vec{a} \in A , \forall \vec{p} \in P: \left|  \left| \vec{a} - \vec{p}  \right| \right|_{2} \leq r \implies (p,a) \in Kanten \end{math}
\end{figure}
...
Der MLP lernt, wie die Gewichte für die oben bestimmten Kanten seinen sollen. Dafür nutz der MLP die relative Position von ausgewählten Punkten zu ihren Nachbarn. Ein MLP funktioniert ähnlich zu dem Regressionsnetz, was in Abschnitt \ref{ Punktwolken Transformation} erklährt wird.
Ein ausgewählter Punkt verändert seine Merkmale anhand der Summe von Mermalen der Punkte in seiner Nachbarschaft, die nach ihrer relativen Position mit dem MLP gewichtet werden. Die Anzahl an Punkten verringert sich, aber die Anzahl der Merkmale pro Punkt erhöht sich mit jeder Schicht.
...
In diesem Fall wird auf die Z-Invarinaz regressiert. Der Merkmalsvektor wird über mehere Schichten auf eine Ausgabe abgeleitet.
...
die Transformation verläslicher zu machen. Es sind voll verbundene Schichten, die in Abschnitt \ref{Punktwolken Transformation} beschrieben wurden.
In jeder Schicht werden Maßnahmen wie Normalisierung und zufälliges zurrücksetzen eingesetzt, um die Regression stabieler zu machen.
...
Das Ergebniss der Regression ist die Z-Invarianz.
Die Z-Invarianz ist aber noch keine Rotationmatrix und muss daher zu einer gemacht werden, um ein Bauteil drehen zu können, 
...
Eine Pubktwokle bestehend aus mehreren Punkten  $(x_{i},y_{i},z_{i})$ kann wie folg durch Matrixmultiplikation mit einer Rotationmatrix $R$ gedreht werden.
...
Um aus dem 3d Vektor, der die Z-invarianz darstellt, eine Rotationsmatrix zu erstellen, kann die Z-Invarianz selber als Zeile der Rotationmatrix genommen werden. Die restlichen Zeilen müssen dann aus der ersten Zeile abgeleitet werden.
...
In der Abbildung \ref{n1} wird die Berechung von der Normale-1 ($n_1$) dargestellt. Die aufgespannt Ebene ist rot gefärbt.
...
In der Abbildung \ref{n2} wird die Berechung von der Normale-2 ($n_2$) dargestellt. Die aufgespannt Ebene ist rot gefärbt.
...
In der Abbilding \ref{Vertauschen} ist der Unterscied, den das Vertauschen von $n_1$ und $n_2$ machen kann dargestellt.
...
In der Abbilding \ref{Veränderung der Rotationmatrix durch die Spiegelmatrix} wird die Spiegelmatrix und die Veränerungen, die sie auf eine Rotationmatrix wirkt dargestellt.

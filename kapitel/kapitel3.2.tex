...
Um ein Bauteil korrekt auszurichten, muss eine Fläche des Bauteils auf eine Ausrichtung abgebildet werden. Die Flächen in den Bauteilen wurden in Kapitel 3 identifieziert und als Punktwolken gespeichert. 
Mit Punktwolke ist in diesem Fall eine Menge an Raumkooredinaten gemeint, die zusammengenommen eine Struktur darstellen. Eine Punktwolke besteht aus endlich vielen Raumkooredinaten, und ist daher immer nur eine Aproximation.
...
Dieses Neurale Netz wird eine Punktwolke beliebiger größe zu der Z-Invarianz transformieren. Die Z-Invarianz wird in einem folgenden Abschnitt erklährt. Aus dieser Ausgabe lässt sicht dann eine vollständige Drehung erstellen.
...
Durch das Anpassen von Gewichten kann ein Neuron den Einfluss von Eingabedaten verändern. Die optimale Belegung von Gewichten wird erlernt. Dafür muss definiert werden, was das Neurale Netz als gutes Ergebnis ansieht. Für das Lernen wird ein Datensatz genutzt, 
den das Neurale Netz mehmal durchläuft. Diese Daten werden Trainingsdaten genannt. Bei jeden Durchlauf der Trainingsdaten
übeprüft das Neurale Netz, wie gut seine Ausgaben im Vergleich zu dem vordefinierten optimalen Ergebnis sind. Neurale Netze funktionieren mit überwachtem Lernen. Das bedeutet, 
dass für jeden Datenpunkt in den Trainingsdaten eine  Lösung vorliegt, gegen die das Neurale Netz seine eigene Ausgabe Testen kann.
Es versucht dann durch Backpropagation zu ermitteln, wo der Großteil der Fehler herkommen, verändert diese Gewichte und wiederholt den gesammten Vorgang, bis entweder eine vorbestimmte Anzahl an Durchläufen abgearbeitet ist oder das Neurale Netz eine vorbestimmte Fehlergrenze unterschreitet.
Der Fehler auf den Trainingsdaten wird Loss genannt. Den Loss zu minimiren ist das Ziel des Neuralen Netzes. %genauer

\smallskip

Backpropagation ist eine Methode, die in Neuralen Netzen eingesetzt wird, um zu bestimen, welches Gewicht wie viel zu dem aktuellen Loss beigetragen hat. Dafür arbeitet es rückwärts von der Ausgabeschicht zu der Eingabeschicht, mittels Gradienten vom Loss. 
Die Schichten eines Neuralen Netzes werden in dem Abschnitt \ref{Regression auf Merkmalsvektor} dargestellt.
...
Die generelle Struktur der Merkmalsextraktion ist eine wiederholte Folge an Reduzierungen auf den konkreten Daten der Punktwolke, gefolg von einem kleinem Neuralen Netz, was mit den konkreten Daten dieser Schicht Merkmale über mehrere Schichten hinweg konstruiert.
...
Durch die wiederholte Reduzierung der Ausgangsdaten wird die Anzahl der echten Punkte immer geringer, wie man in der Abbildung \ref{Merkmalsextraktion aus Punktwolke} sehen kann.

Die gerade beschriebene Untermenge an Punkten wird weiterverarbeitet, indem zu jeden Punkt in der Untermenge eine Kante zu erstellt wird, wenn ein anderer Punkt innerhalb von einer bestimmten Distanz des ausgewählten Punktes ist. 
Danach ist jeder ausgewählte Punkt mit allen Punkten einem Radius um sich selber verbunden, diese Punkte sind die jeweiligen Nachbarn der ausgewählten Punkte.
...
Eine Schicht an künstlichen Neuronen ist so definiert, dass man die Neuronen der Schicht immer mit der gleichen Anzahl an Kantenübergägen erreicht. Jedes Neuron der ersten verborgenen Schicht kann zum Beispiel mit nur genau einem Kantenübergang erreicht werden.
...
Diese Schicht repräsentiert die Eingaben. Jeder Knoten der Eingabeschicht ist ein Datenpunkt, oder in diesem Fall ein Merkmal. Danach gibt es die versteckten Schichten. Ihre Aufgabe ist es die Eingabe zu transformieren. 
Sie erlauben es dem Neuralen Netz eine Eingabe über mehrere Schichten zu verarbeiten. Dadurch kann das Neurale Netz nicht lineare zusammenhänge darstellen. Zuletzt gibt es die Ausgabeschicht. Von dieser Schicht wird die Z-Invarianz abgelesen.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{bilder/TanhFunction.jpg}
    \caption[Finale Aktivierungsfunktion des Regressionsnetzes]{Finale Aktivierungsfunktion des Regressionsnetzes}
            \label{Tanh Aktivierungsfunktion des Regressionsnetzes}
    \quelle\url{https://images.squarespace-cdn.com/content/v1/5acbdd3a25bf024c12f4c8b4/1524687495762-MQLVJGP4I57NT34XXTF4/TanhFunction.jpg}
\end{figure}

Um die Z-invarinaz immer im korrekten Wertebereich zu haben wird als Finale Aktivierungsfunktion Tanh genutzt. Diese Funktion limitiert die Ausgaben auf -1 bis 1 wie man in der Abbildung \ref{Tanh Aktivierungsfunktion des Regressionsnetzes} sehen kann, 
was genau die Werte sind, die ein Eintrag in einer Rotationmatrix haben kann.
...
Wenn sie vor der Spiegelung nicht im 180 Grad Halbkreis der erlaubten Drehungen war, dann ist sie es danach. Testen

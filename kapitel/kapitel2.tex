...
Bevor ein Objekt gedruckt werden kann, muss es in Schichten aufgeteilt werden. Das wird vor dem Drucken von Software erledigt. Für jede erstellte Schicht wird auf im Vorhinein ein Weg berechnet, mit dem die Düse die Schicht abfährt. 
...
Der Pfad, den die Düse über eine Schicht nimmt, kann ebenfalls optimiert werden. Ein Düsenpfad (engl. "Toolpath") kann die Qualität und Geschwindigkeit stark beeinflussen \cite{zhuang2024comprehensive}. Ein längerer Pfad, den die Düse abarbeiten muss, resultiert in einer längeren Druckzeit.

Topologieoptimierung kann ebenfalls Druckzeit einsparen, es wird in Abschnitt \ref{Topologieoptimierung} vorgestellt.

\section{Topologieoptimierung}
...
\begin{figure}[t]
    \centering
    \includegraphics[scale=1]{bilder/Topologie.png}
    \caption[Darstellung von durch Topologieoptimierung aufgeteiltem Bauteil (links) und verbesserte Aufteilung durch Topologieoptimierung (rechts)]{Darstellung von durch Topologieoptimierung aufgeteiltem Bauteil (links) und verbesserte Aufteilung durch Topologieoptimierung (rechts)}
            \label{Darstellung von durch Topologieoptimierung aufgeteiltem Bauteil (links) und verbesserte Aufteilung durch Topologieoptimierung (rechts)}
    \quelle\url{https://media.springernature.com/lw685/springer-static/image/art%3A10.1186%2Fs42492-018-0004-3/MediaObjects/42492_2018_4_Fig9_HTML.png?as=webp}
\end{figure}


Das generelle vorgehen ist es ein Objekt in kleinere Teile zu unterteilen. Die Teile, in die es unterteilt wird, können unterschiedliche Formen haben in der Abbildung sind sie als Vierecke dargestellt. Die Schwarzen Kanten sind die Teile des Bauteils, die gedruckt werden. 
Dazwischen kann man weiße Freiräume sehen. In der linken Aufteilung in Abbildung  ist das Bauteil in gleich große Rechtecke aufgeteilt. Die strukturelle stärke des Bauteils ist abhängig von der Größe der Aufteilung. 
Weniger internes Material sorgt für geringere strukturelle Stärke. Um die strukturelle Stärke an Bereichen mit höherer Belastung zu verstärken, kann die Dichte in benötigten Bereichen erhöht werden. 
In der Abbildung ist auf der linken Seite ein Bauteil mit zwei unterschiedlich dichten Aufteilungen zu sehen. In dem Zentrum des Bauteils, wo die meiste Belastung zu erwarten ist, 
...
\begin{figure}[t]
    \centering
    \includegraphics[scale=1.4]{bilder/5d-Drucker.jpg}
...
Auf gekrümmten Flächen ist es schwer Material zu platzieren. Mögliche Lösungsansätze sind bessere Kontrolle der Düse verbesserte Haftung zwischen Schichten oder optimale und stabile Temperatur des Druckmaterials beim austritt aus der Düse \ref{kumar2026recent}.
...
Bei dem 3d-Druck ist das nicht möglich, da die Düse immer über dem Druckbereich ist. Es macht die Planung des Düsenpfades schwerer, da die Reihenfolge der zu druckenden Flächen berücksichtigen muss, dass eine Fläche unter Umständen nicht erreichbar ist, 
...
Der 5d Druck bietet die Möglichkeit komplexere Komponenten mit weniger Material zu drucken. Außerdem sind 5d gedruckte Teile stabiler. Allerdings ist es eine neue Technologie und es gibt noch mehrere technische Probleme die gelöst werden müssen.
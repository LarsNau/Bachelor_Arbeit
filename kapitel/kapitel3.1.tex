...
Das Ziel der Ausrichtungen ist es eine Fläche senkrecht in Relation zu der Druckerplatte zu Orientieren. Bei einer gekrümmten Fläche ist dies unmöglich, wenn man zwei Tangenten an zwei zufälligen Punkten auf einer gekrümmten Fläche nimmt, 
werden diese nicht parallel sein. Die Tangenten haben die Steigung der Fläche an den Punkten. Die Abbildung \ref{Darstellung von Tangenten in einer gekrümmten Fläche} zeigt eine gekrümmte Fläche in Blau und auf der Fläche zwei Tangenten, $t_1$ und $t_2$, 
...
Viele Methoden des Clusterings benötigen eine vorgegebene Anzahl an Gruppen, in die sie die Punkte einteilen sollen. Es muss davon ausgegangen werden, dass bei einem Bauteil die Anzahl der Flächen die gefunden werden können unbekannt sind.
Das macht Methoden, die eine Anzahl an Gruppen benötigen, ungeeignet. Stattdessen wird eine Methode genutzt, in der alle Datenpunkte zum Start ihre eigene Gruppe haben, sodass es für jeden Datenpunkt eine Gruppe gibt. 
Die zwei Gruppen, die sich am ähnlichsten sind, werden dann zusammenfügt.
Auf diesen Gruppen wird der Vorgang wiederholt, bis das Zusammenfügen von Gruppen immer einen vor der Ausführung festgelegten Differenzwert überschreitet. Es passten sich so die Anzahl der Gruppen dem Bauteil an. 
...
beiden einzelnen Gruppen abgezogen, das Ergebnis ist die Erhöhung der Varianz durch das Zusammenfügen von A und B. Die Gruppen zwischen denen diese Erhöhung minimal ist, werden zusammengefasst. % was ist Varianz
...
Wenn weniger als $minPts$ innerhalb des Radius sind, dann gilt der Punkt als Ausnahmepunkt (engl. Noise). Das bedeutet aber nicht unbedingt, dass der Punkt Noise bleibt. Er kann auch zu einem Randpunkt eines Noch nicht existierenden Clusters werden.

...
Das Ziel der Ausrichtungen ist es eine Fläche senkrecht in Relation zu der Druckerplatte zu Orientieren. Bei einer gekrümmten Fläche ist dies unmöglich, wenn man zwei Tangenten an zwei zufälligen Punkten einer gekrümmten Fläche nimmt, 
...
In der Abbildung \ref{Clustering von verstreuten zweidimensionalen Punkten} sind zwei zweidimensionale Punktwolken zu sehen. Die linke ist nicht geclustert, die rechte ist geclustert, beide stellen dieselbe Punktwolke dar.
...
der, zu der Oberfläche des Bauteils, an der sich der Punkt befindet, senkrecht steht. Solche Vektoren werden Normalvektor genannt. Es sind nur die Normalvektoren relevant, 
...
Punkte mit ähnlichem Normalvektor haben eine ähnliche Ausrichtung und sollten teil derselben Fläche sein. Die Darstellung als dreidimensionale Punkte erlaubt funktioniert nur, wenn alle Normalvektoren normiert sind. Ein normierter Vektor hat eine Länge von 1. 

\subsection{Hierarchisches Clustering}
\label{Hierarchisches Clustering}

Die in Abschnitt \ref{Normalvektoren} beschriebenen Normalvektoren werden durch eine Clustering-Verfahren in diesem Abschnitt in Gruppen unterteilt. Die dadurch entstehenden Cluster enthalten jeweils Punkte mit ähnlichen Normalvektoren. Ein Cluster, dass Punkte mit ähnlichen Normalvektoren enthält,
...
Viele Methoden des Clusterings benötigen eine vorgegebene Anzahl an Gruppen, in die sie die Punkte einteilen sollen. Es muss davon ausgegangen werden, dass bei einem Bauteil die Anzahl der Flächen die gefunden werden können unbekannt ist.
...
Aber bei komplexen Bauteilen ist es notwendig einen höheren Fehler zuzulassen, um gebogene Flächen zu finden. Bei komplexen Bauteilen würde die Suche nach ausschließlich gerade Flächen in sehr viele extrem kleinen Flächen resultieren, 
